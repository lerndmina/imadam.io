...
## ViewTransitions
I decided to try out the new view transitions feature that Astro 3.0 introduced. View transitions allow you to create smooth animations between pages using the native CSS `@view-transition` rule. This feature is currently only supported by Chromium browsers, but Astro provides a fallback mechanism for other browsers. 
